...
class agg(nn.Module):
    def __init__(self, c=32):
        ...
        self.left = FusionAttention(c)
        self.right = FusionAttention(c)
        self.center = FusionAttention(c)
        self.up1 = PixUpBlock(c)
        self.up2 = PixUpBlock(c)
        self.up3 = PixUpBlock(c)

    def forward(self, x, y, z):
        xy = self.up1(self.left(x, y))
        yz = self.up2(self.right(y, z))
        xz = self.up3(self.center(x, z))
...
class MSSOD(nn.Module):
    def __init__(self, rfb_out_channel=32):
        ...
        self.out_conv = nn.Conv2d(2*5, 1, 3, 1, 1)
    ...
    def forward(self, low_input, high_input):
        ...
        fa_1, fa_2, fa_3 = self.aggr(fa_1, fa_2, fa_3)


        sum_c = torch.cat((fa_1, center, fa_2, center, fa_3), dim=1) # c=32*3
